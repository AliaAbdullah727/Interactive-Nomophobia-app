...
# Reliable scroll control on Streamlit Cloud (uses an invisible iframe + focus trick)
...
def init_state() -> None:
    ...
    # Scroll flags (executed on NEXT run; reliable with Streamlit reruns)
    st.session_state.setdefault("scroll_to_top", False)


def maybe_scroll_top() -> None:
    """If flagged, scroll to the top (must run at the TOP of the render)."""
    if st.session_state.get("scroll_to_top"):
        scroll_to_here(0, key="top")  # 0ms delay; adjust if you want animation
