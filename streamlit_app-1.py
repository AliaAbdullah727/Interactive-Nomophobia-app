...
SEVERITIES = {
    ...
    "Moderate": Severity(
        ...
        recs=[
            ...
            "Create a 'charging routine': charge before leaving and carry a small power bank if needed.",
...
def request_scroll_to_top() -> None:
    """Set a flag so the NEXT render scrolls to the top.

    Doing it *before* st.rerun() often doesn't work because rerun interrupts execution.
    """
    st.session_state["__scroll_top__"] = True


def maybe_scroll_to_top() -> None:
    """If the flag is set, inject JS now (during the new run) then clear the flag."""
    if not st.session_state.get("__scroll_top__", False):
        return
    st.session_state["__scroll_top__"] = False
    components.html(
        """
        <script>
...
def fig_gauge(total: int, color: str) -> go.Figure:
    fig = go.Figure(
        go.Indicator(
            mode="gauge+number",
            value=total,
...
def fig_subscale_bar(subscores: Dict[str, int]) -> go.Figure:
...
def init_state() -> None:
    ...
    st.session_state.setdefault("__scroll_top__", False)
...
def render_survey() -> None:
    # If we changed pages in the previous run, jump to top now
    maybe_scroll_to_top()
    ...
    for i in range(start, end + 1):
        ...
        # Default to Neutral (4) if unanswered
        if i not in answers:
            answers[i] = 4

        default = answers.get(i, 4)
        idx = values.index(default) if default in values else values.index(4)
...
def render_results() -> None:
    ...
    with c1:
        st.plotly_chart(fig_gauge(total, sev.color), use_container_width=True)
    with c2:
        st.plotly_chart(fig_subscale_bar(subs), use_container_width=True)
